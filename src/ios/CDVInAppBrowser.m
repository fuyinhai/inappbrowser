...
-(void)closeOfHide
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        UIView *subviews  = [weakSelf.viewController.view viewWithTag:6677];
...
- (void)show:(CDVInvokedUrlCommand*)command
{
    ...
    [self.inAppBrowserViewController setCloseButtonTitle:@"返回了"];
    self.inAppBrowserViewController.CloseOfHideDelegate =self;
...
{
    if (self.inAppBrowserViewController == nil) {
        NSLog(@"inAppBrowserViewController  还未创建");
        return;
    }
...
- (void)close
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self.CloseOfHideDelegate != nil)
        {
            [self.CloseOfHideDelegate closeOfHide];
        }
